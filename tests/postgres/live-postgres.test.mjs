...
test("real persistence keeps raw/canonical JSON separate and warning nonblocking", async () => { await withDatabase(async (client) => {
  await runMigrations(client); await client.query("insert into import_batch (import_id,schema_version,status) values ($1,$2,'RECEIVED')",["LIVE-1","test-v1"]);
...
test("real persistence marks blocking errors invalid and fails batch", async () => { await withDatabase(async (client) => {
  await runMigrations(client); await client.query("insert into import_batch (import_id,schema_version,status) values ($1,$2,'RECEIVED')",["LIVE-2","test-v1"]);
...
test("real PostgreSQL transaction rolls back partial writes on injected failure", async () => { await withDatabase(async (client) => {
  await runMigrations(client); await client.query("insert into import_batch (import_id,schema_version,status) values ($1,$2,'RECEIVED')",["LIVE-3","test-v1"]);
...
test("live import lifecycle and query API works after clean bootstrap", async () => { await withDatabase(async (client) => {
  await runMigrations(client); const created=await createImportBatch(client,{importId:"LIVE-Q-1",schemaVersion:"test-v1"}); assert.equal(created.status,"RECEIVED");
...
test("live orchestration persists a successful two-row CSV import", async () => { await withDatabase(async (client) => {
  await runMigrations(client); const result=await runRecordImport({db:client,importId:"LIVE-O-1",contract:orchestrationContract,csvText:"id,name\n1,Alice\n2,Bob\n",transform:orchestrationTransform,getRecordId:orchestrationRecordId});
...
test("live orchestration durably records a pre-staging CSV failure", async () => { await withDatabase(async (client) => {
  await runMigrations(client); const result=await runRecordImport({db:client,importId:"LIVE-O-2",contract:orchestrationContract,csvText:'id,name\n1,"Alice\n',transform:orchestrationTransform,getRecordId:orchestrationRecordId});
...
test("live filesystem import persists a successful CSV file", async () => withTempDir(async (dir) => { await withDatabase(async (client) => {
  await runMigrations(client); const filePath=join(dir,"records.csv"); await writeFile(filePath,"id,name\n1,Alice\n2,Bob\n","utf8");
  const result=await runRecordFileImport({db:client,importId:"LIVE-F-1",contract:orchestrationContract,filePath,transform:orchestrationTransform,getRecordId:orchestrationRecordId});
...
test("live filesystem import durably records a missing-file failure", async () => withTempDir(async (dir) => { await withDatabase(async (client) => {
  await runMigrations(client); const result=await runRecordFileImport({db:client,importId:"LIVE-F-2",contract:orchestrationContract,filePath:join(dir,"missing.csv"),transform:orchestrationTransform,getRecordId:orchestrationRecordId});
...
test("live filesystem import preserves downstream row-error semantics", async () => withTempDir(async (dir) => { await withDatabase(async (client) => {
  await runMigrations(client); const filePath=join(dir,"invalid-row.csv"); await writeFile(filePath,"id,name\n1,Alice\n","utf8");
  const result=await runRecordFileImport({db:client,importId:"LIVE-F-3",contract:orchestrationContract,filePath,transform:orchestrationTransform,getRecordId:orchestrationRecordId,diagnose:()=>[{code:"LIVE_FILE_INVALID_NAME",severity:"ERROR",fieldKey:"name",detail:"Synthetic filesystem blocking diagnostic."}]});
